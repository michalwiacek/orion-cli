...
pub fn run(allocator: std.mem.Allocator, args: []const []const u8) !void {
    if (args.len < 1) {
        std.debug.print(
            ...
            \\  orion curl get:/api/v1/offers/{{id}} --param id=123
            \\  orion curl post:/api/v1/auth/register --body '{{"email":"a@b.com","password":"x"}}'
    ...
    const method_upper = try toUpperOwned(allocator, resolved_target.method_name);
    defer allocator.free(method_upper);

    const quoted_url = try shellSingleQuote(allocator, resolved_target.url);
    ...
    try tokens.append(allocator, try allocator.dupe(u8, "curl"));

    for (parsed.curl_flags) |flag| {
        try tokens.append(allocator, try allocator.dupe(u8, flag));
    }
    try tokens.append(allocator, try allocator.dupe(u8, "-X"));
    try tokens.append(allocator, try allocator.dupe(u8, method_upper));
    try tokens.append(allocator, try allocator.dupe(u8, quoted_url));

    if (body.slice) |payload| {
        if (!resolved_target.method.requestHasBody()) {
            printCurlError(error.MethodDoesNotSupportBody);
            return;
        }
        const quoted_content_type = try shellSingleQuote(allocator, "Content-Type: application/json");
        defer allocator.free(quoted_content_type);
        const quoted_payload = try shellSingleQuote(allocator, payload);
        defer allocator.free(quoted_payload);

        try tokens.append(allocator, try allocator.dupe(u8, "-H"));
        try tokens.append(allocator, try allocator.dupe(u8, quoted_content_type));
        try tokens.append(allocator, try allocator.dupe(u8, "--data-raw"));
        try tokens.append(allocator, try allocator.dupe(u8, quoted_payload));
    }

    const rendered = if (parsed.pretty)
        try renderPretty(allocator, tokens.items)
    else
...
const CallArgs = struct {
    ...
    fn deinit(self: *CallArgs, allocator: std.mem.Allocator) void {
        ...
        for (self.curl_flags) |f| allocator.free(f);
        allocator.free(self.curl_flags);
        self.* = undefined;
    }
};
...
fn parseCallArgs(allocator: std.mem.Allocator, args: []const []const u8) !CallArgs {
    ...
    var curl_flags: std.ArrayList([]u8) = .{};
    defer curl_flags.deinit(allocator);
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (std.mem.eql(u8, arg, "--curl-flag")) {
            if (i + 1 >= args.len) return error.MissingCurlFlagValue;
            i += 1;
            try curl_flags.append(allocator, try allocator.dupe(u8, args[i]));
            continue;
        }
...
fn toUpperOwned(allocator: std.mem.Allocator, raw: []const u8) ![]u8 {
    const out = try allocator.dupe(u8, raw);
    for (out) |*c| c.* = std.ascii.toUpper(c.*);
    return out;
}

fn shellSingleQuote(allocator: std.mem.Allocator, raw: []const u8) ![]u8 {
    var out: std.ArrayList(u8) = .{};
    defer out.deinit(allocator);
...
fn renderSingleLine(allocator: std.mem.Allocator, tokens: []const []u8) ![]u8 {
    var out: std.ArrayList(u8) = .{};
    defer out.deinit(allocator);
    for (tokens, 0..) |t, idx| {
        if (idx != 0) try out.append(allocator, ' ');
...
fn renderPretty(allocator: std.mem.Allocator, tokens: []const []u8) ![]u8 {
    var out: std.ArrayList(u8) = .{};
    defer out.deinit(allocator);
    if (tokens.len == 0) return allocator.dupe(u8, "");
    try out.appendSlice(allocator, tokens[0]);
    for (tokens[1..]) |t| {
        try out.appendSlice(allocator, " \\\n  ");
        try out.appendSlice(allocator, t);
    }
